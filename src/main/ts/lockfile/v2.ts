...
export const patchEntry = (entry: TLockfileEntry, name: string, newVersion: string): TLockfileEntry => {
  ...
  // NOTE seems like deps are not updated by `yarn mode='--update-lockfile'`
  const yarnVersion = invoke('yarn', ['--version'], process.cwd(), true, false)
  const args = semver.gte(yarnVersion, '2.0.0')
    ? ['npm', 'info', `${name}@${newVersion}`, '--fields', 'dependencies', '--json']
    : ['info', `${name}@${newVersion}`, 'dependencies', '--json']

  const dependencies = JSON.parse(invoke('yarn', args, process.cwd(), true, false))
...
export const parseAuditReport = (
  ...
  Object.values(JSON.parse(data).advisories).reduce<TAuditReport>((m, {vulnerable_versions, module_name, patched_versions}: any) => {
    m[module_name] = {
      patched_versions,
      vulnerable_versions,
      module_name,
