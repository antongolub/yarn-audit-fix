...
describe('patch', () => {
  it('yarnlock v2', () => {
    const report = fs.readFileSync(join(fixtures, 'lockfile/v2/yarn-audit-report.json'), 'utf-8')
    const lockfile = fs.readFileSync(join(fixtures, 'lockfile/v2/yarn.lock'), 'utf-8')
    const expected = fs.readFileSync(join(fixtures, 'lockfile/v2/yarn-lock-patched.yaml'), 'utf-8')
    const result = format(patch(
